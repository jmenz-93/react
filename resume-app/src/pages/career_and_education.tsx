      ...
      <div className="py-10 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          ...
          <section className="mb-20 text-center pt-8">
            <h2 className="text-4xl sm:text-5xl font-bold text-slate-900 dark:text-slate-100 mb-6 tracking-tight">
              Hi, I'm Jonathan.
            </h2>
        ...
        <section className="mb-20">
          <div className="flex items-center mb-8">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-blue-100/60 dark:bg-blue-900/30">
                <BriefcaseIcon className="h-6 w-6 text-blue-600 dark:text-blue-400" />
            </div>
            <h2 className="ml-4 text-sm font-bold uppercase tracking-widest text-slate-600 dark:text-slate-400">Work Experience</h2>
          </div>
          <div className="relative ml-6 border-l-2 border-slate-300 dark:border-slate-600 pl-8 py-2">
            {workExperience.map(job => (
              <div key={job.id} className="relative mb-12">
                <span className={`absolute -left-[2.6rem] top-6 h-5 w-5 rounded-full border-4 border-white dark:border-slate-900 ${job.isCurrent ? 'bg-blue-600 ring-2 ring-blue-100 dark:ring-blue-900' : 'bg-slate-400 dark:bg-slate-600 ring-2 ring-slate-100 dark:ring-slate-800'}`} />
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 hover:border-slate-300 dark:hover:border-slate-600 group">
                  <div className="flex items-center mb-4">
                    <NMLogo imageUrl={job.imageUrl} linkUrl='https://www.northwesternmutual.com/' altText={`${job.title} logo`} />
                    <div className="ml-4">
                      <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 transition-colors">{job.title}</h3>
                      <p className="text-sm font-medium text-slate-500 dark:text-slate-400">{job.dates}</p>
                    </div>
                  </div>
                  <ul className="list-disc pl-5 space-y-2 text-sm text-slate-600 dark:text-slate-300 leading-relaxed marker:text-slate-400 dark:marker:text-slate-500">
                    {job.duties.map((duty) => <li key={duty}>{duty}</li>)}
                  </ul>
                  <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-700">
                      <div className="flex flex-wrap gap-2">
                        {job.technologies.map(tech => (
                          <span key={tech} className="px-2.5 py-1 text-xs font-medium text-blue-700 dark:text-blue-200 bg-blue-50 dark:bg-blue-900/50 rounded-md border border-blue-100 dark:border-blue-800">
                            {tech}
                          </span>
                        ))}
                      </div>
          ...
          <section className="mb-20">
            <div className="flex items-center mb-8">
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-indigo-100/60 dark:bg-indigo-900/30">
                <AcademicCapIcon className="h-6 w-6 text-indigo-600 dark:text-indigo-400" />
              </div>
              <h2 className="ml-4 text-sm font-bold uppercase tracking-widest text-slate-600 dark:text-slate-400">Education</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {Object.entries(educationDetails).map(([key, details]) => (
                <article key={key} className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 hover:border-slate-300 dark:hover:border-slate-600 group h-full flex flex-col">
                  <div className="flex items-center mb-4">
                    <div className="mr-4 flex-shrink-0">
                      {details.logo}
                    </div>
                    <div>
                      <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 transition-colors">{details.title}</h3>
                      <p className="text-sm text-slate-500 dark:text-slate-400">{details.degree}</p>
                    </div>
                  </div>
          ...
          <section>
            <div className="flex items-center mb-8">
                ...
                <h2 className="ml-4 text-sm font-bold uppercase tracking-widest text-slate-600 dark:text-slate-400">Tech Certifications</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {/* AWS Card */}
                <article className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm cursor-pointer transition-all duration-300 h-full hover:shadow-lg hover:-translate-y-1 hover:border-slate-300 dark:hover:border-slate-600 group">
                    <div className="flex items-center">
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 transition-colors">AWS Certified AI Practitioner</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400">Issued by Amazon Web Services</p>
                        </div>
                    </div>
                </article>
                    ...
                    <div className="flex items-center">
                        ...
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 transition-colors">CompTIA Security+</h3>
                                <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-200 tracking-wide">Studying</span>
                            </div>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mb-2">Issued by CompTIA</p>
                        </div>
                    </div>
                </article>
          </div>
